...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        System.out.println(firstFiveStates);
        ...
        List<Integer> firstFiveNumbers = IntStream.range(1, 20).boxed().collect(Collectors.toList());
        ...
        StateInfo cardinalState = null;//states.stream().filter(filter -> filter.getStateBird().equals("cardinal")).findFirst().orElseThrow();
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream().anyMatch(lowest -> lowest.getLowestElevationInFeet() < 0);
        ...
        Boolean isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(highest -> highest.getHighestElevationInFeet() > 21000);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(anthem -> anthem.getStateAnthem().equals(true));
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(mot -> mot.getStateMotto().split(" ").length == 1);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(averagingDouble(average -> average.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(total -> total.getCapital().getAverageYearlyPrecipitationInInches()).sum();
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(Collectors.groupingBy(zone -> zone.getTimeZones() + " ", counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(Collectors.groupingBy(zone -> zone.getCapital().getTimeZone(), counting()));
        ...
        List<String> stateTreesSortedAscending = states.stream().map(StateInfo::getStateTree).sorted().collect(Collectors.toList());
        ...
        List<String> allDistinctStateBirds = states.stream().map(StateInfo::getStateBird).distinct().collect(Collectors.toList());
